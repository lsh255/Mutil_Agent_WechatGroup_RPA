...
**Dockerfile.test** 提供了轻量级的测试环境镜像，用于快速开发和测试。
...
**测试镜像特点**：
- 基于 Python 3.12 基础镜像，不包含完整微信环境
- 仅包含必要的依赖（OpenCV、FastAPI、Redis 等）
- 适合单元测试和 API 测试
- 构建速度快，资源占用低
...
**VNC 远程访问**：
- **noVNC Web界面**：通过浏览器访问（端口 6080/6081/6082/6083），密码默认为 `wechat123`
- **VNC客户端**：支持 RealVNC、TightVNC 等客户端（端口 5900/5901/5902/5903），密码默认为 `wechat123`
